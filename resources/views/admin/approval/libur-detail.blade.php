{{-- resources/views/admin/approval/libur-detail.blade.php --}}
...
@section('page-subtitle', 'Review pengajuan libur pengganti karyawan')
...
<style>
    ...
    .card-header i.orange { background: linear-gradient(135deg, #FF6B35 0%, #F7931E 100%); }
    ...
    .reject-modal-overlay.show { display: flex; animation: overlayIn 0.25s ease forwards; }
    .reject-modal-overlay.hide { display: flex; animation: overlayOut 0.25s ease forwards; }
...
<div class="approval-detail-wrapper">
    ...
    <div class="detail-content">
        ...
        <div class="detail-card">
            ...
            <div class="card-body">
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ $libur->karyawan->user->nama }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Departemen</span>
                    <span class="detail-value">{{ $libur->karyawan->departemen->nama }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="detail-card">
            <div class="card-header">
                <i class="fas fa-calendar-check orange"></i>
                <span>Informasi Pengajuan</span>
            </div>
            <div class="card-body">
                <div class="detail-row">
                    <span class="detail-label">Tanggal Libur</span>
                    <span class="detail-value">{{ $libur->tanggal->format('d F Y') }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Diajukan</span>
                    <span class="detail-value">{{ $libur->created_at->format('d F Y, H:i') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="detail-card">
            ...
            <p class="reason-text">{{ $libur->alasan }}</p>
        </div>
        ...
        <div class="detail-card">
            ...
            <div class="timeline">
                @foreach($libur->approvals as $approval)
    ...
    @php
        ...
        $isYourTurn = $libur->status === 'pending' && $libur->current_step === $userRole;
    @endphp
    ...
    <div class="detail-actions">
        ...
        @elseif($libur->status === 'pending')
            @php
                $currentLabel = [
                    'admin' => 'Admin Departemen',
                    'gm'    => 'General Manager',
                    'super_admin' => 'Super Admin'
                ][$libur->current_step] ?? strtoupper($libur->current_step);
            @endphp
        ...
        @else
            <button type="button" class="btn btn-disabled" disabled>
                <i class="fas fa-lock"></i> Sudah {{ ucfirst($libur->status) }}
            </button>
        @endif
    </div>
</div>
...
<div id="rejectModal" class="reject-modal-overlay" onclick="handleOverlayClick(event)">
    <div class="reject-modal-box" onclick="event.stopPropagation()">
        <div class="reject-modal-head">
            ...
            <h3>Tolak Pengajuan Libur</h3>
        </div>
        <form id="rejectForm" method="POST" action="{{ route('admin.libur.reject', $libur->id) }}">
            @csrf
...
<script>
    ...
    function confirmApprove() {
        showAlert('warning', 'Setujui Libur Pengganti?', 'Anda yakin ingin menyetujui pengajuan libur pengganti ini?', function() {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `{{ route('admin.libur.approve', $libur->id) }}`;
            form.innerHTML = `@csrf`;
