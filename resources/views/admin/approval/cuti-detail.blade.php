{{-- resources/views/admin/approval-cuti-detail.blade.php --}}

@extends(is_admin_mode()
    ? 'admin.layouts.app'
    : 'karyawan.layout.fullscreen')

@section('title', 'Detail Approval Cuti')
@section('page-title', 'Detail Approval Cuti')
@section('page-subtitle', 'Review pengajuan cuti karyawan')

@push('styles')
<style>
    /* ===== WRAPPER ===== */
    /* Desktop: normal flow, centered */
    ...
    /* ===== HEADER ===== */
    /* Desktop: sembunyikan karena sudah ada page title di admin layout header */
    .detail-header {
        display: none;
    }

    /* Mobile: tampilkan karena karyawan layout tidak punya page title */
    @media (max-width: 768px) {
        .detail-header {
            display: flex;
            align-items: center;
    ...
    /* Desktop: actions mengalir di bawah konten */
...
<div class="approval-detail-wrapper">
    ...
    <div class="detail-content">
        ...
        <div class="detail-card">
            ...
            <div class="card-body">
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ $cuti->karyawan->user->nama }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Departemen</span>
                    <span class="detail-value">{{ $cuti->karyawan->departemen->nama }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Jabatan</span>
                    <span class="detail-value">{{ $cuti->karyawan->jabatan->nama ?? '-' }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="detail-row">
                    ...
                    <span class="detail-value">
                        <span class="badge-orange">{{ $cuti->jenisCuti->nama }}</span>
                    </span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Tanggal Mulai</span>
                    <span class="detail-value">{{ $cuti->tanggal_mulai->format('d F Y') }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Tanggal Selesai</span>
                    <span class="detail-value">{{ $cuti->tanggal_selesai->format('d F Y') }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Durasi</span>
                    <span class="detail-value">{{ $cuti->jumlahHari }} hari</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Diajukan</span>
                    <span class="detail-value">{{ $cuti->created_at->format('d F Y, H:i') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="detail-card">
            ...
            <p class="reason-text">{{ $cuti->alasan }}</p>
        </div>
        ...
        <div class="detail-card">
            <div class="card-header">
                <i class="fas fa-file gray"></i>
                <span>File Pendukung</span>
            </div>
            <a href="{{ Storage::url($cuti->file_pendukung) }}" target="_blank" class="file-link">
        ...
        <div class="detail-card">
            ...
            <div class="timeline">
                @foreach($cuti->approvals as $approval)
                    <div class="timeline-item">
                        ...
                        <div class="timeline-info">
                            ...
                            @endif
                            @if($approval->catatan_admin)
                                <div class="timeline-note">"{{ $approval->catatan_admin }}"</div>
                            @endif
                        </div>
                    </div>
                @endforeach
    ...
    <!-- Action Buttons -->
    @php $isYourTurn = $cuti->current_step === Auth::user()->role; @endphp
    <div class="detail-actions">
        ...
        @else
            @php
                ...
                ][$cuti->current_step] ?? strtoupper($cuti->current_step);
            @endphp
...
<div id="rejectModal" class="reject-modal-overlay" onclick="handleOverlayClick(event)">
    <div class="reject-modal-box" onclick="event.stopPropagation()">
        ...
        <form id="rejectForm" method="POST" action="{{ route('admin.cuti.reject', $cuti->id) }}">
            @csrf
...
<script>
    ...
    function confirmApprove() {
        showAlert('warning', 'Setujui Pengajuan Cuti?', 'Anda yakin ingin menyetujui pengajuan cuti ini?', function() {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `{{ route('admin.cuti.approve', $cuti->id) }}`;
